...
bool Player::withdrawCost(int cost) {
    if (foodBank >= cost) {
        this->foodBank -= cost;
        //cout<<"current bal = " << this->foodBank<<endl;
        return true;
    }
    else{
...
Ant& Player::generateAnt(Game * game) {
    Ant *choix = nullptr; // Creates a new ant
    // choiceAnt and choiceLocation
    bool purchasable = false;
    if (this->foodBank <= 1){
        cout<<"---** Unable to buy an ant: low food bank!!! **----"<< endl;
    }
    else{
        while (!purchasable) {
...
int Player::displayList() {
    ...
    cout << "Current food bank: " << this->foodBank << endl;
